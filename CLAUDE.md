...
- `.agency/do.md` — config for the `/do` slash command (check/format/test/CI commands). Check and CI both run `uv run scripts/check.py` (the script's PEP 723 inline metadata declares its PyYAML dep, so `uv run` auto-installs it; `python3 scripts/check.py` works too if PyYAML is system-installed). Format runs `prettier --write '**/*.md'`. Update when commands change.
